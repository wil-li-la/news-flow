...
export default function SettingsScreen() {
  ...
  const refreshUser = useCallback(async () => {
    // Local stub auth - no need to check external auth
    if (user) {
      setStep({ kind: 'done' });
    } else {
      setStep({ kind: 'signIn' });
  ...
  // Navigate to main app when user signs in
  useEffect(() => {
  ...
  const onSignIn = useCallback(async () => {
    ...
    setLoading(true);
    try {
      console.log('Using local stub auth');
      setUser({ userId: 'local-user', username });
      setStep({ kind: 'done' });
    } catch (e: any) {
